    ...
    def __init__(
        ...
        if 'qc' not in data_path:
            ...
            if stat_path is not None:
                summary_stats_k562 = pd.read_excel(stat_path, sheet_name='TabB_K562_day6_summary_stat')
                # remove week perturbation gene and week perturbed cell
                strong_perts = get_strong_perts(summary_stats_k562) + ['non-targeting']
                self.adata = self.adata[self.adata.obs['gene'].isin(strong_perts)]
        ...
        dosage_df = self._get_additional_dosage(dosage_df, criteria='FC3') # FC2.5
        self.d_var_info = dosage_df.T[[]]
        D = torch.from_numpy(dosage_df.to_numpy().astype(np.float32))
        if mask_test is not None:
            print('############ Ablation for unseen perturbation ############')
            # mask_ls = self.d_var_info.index[[322,276,298]].tolist() # out small
            self.mask_test = mask_test
            mask_ls = self.d_var_info.index[mask_test].tolist() # out large
            # mask_ls = ['INTS2', 'HSPA9', 'SUPT5H']
...
def get_ctrl_pair_annotation(adata_origin, anno="matched_ctrl_idx", mode="ot"):
    ...
    if f'{anno}_{mode}' in adata_origin.obs.columns: #  or f'matched_ctrl_idx_0_not_norm' in adata_origin.obs.columns
        print('### DONE : It already has a ctrl pair!!!! ###')
        return adata_origin
    ...
    adata_origin.write_h5ad(f"./datasets/k562_qc_deg_{anno}_{mode}.h5ad")
