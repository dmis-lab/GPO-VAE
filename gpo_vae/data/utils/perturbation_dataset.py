...
class TensorPerturbationDataset(PerturbationDataset):
    ...
    def __getitem__(self, idx: int) -> PerturbationDataSample:
        return dict(idx=idx, X=self.X[idx], D=self.D[idx], qc=self.qc[idx], cfX=self.cfX[idx], deltaX=self.deltaX[idx]) # deltaX = self.deltaX[idx]
...
class SCRNASeqTensorPerturbationDataset(TensorPerturbationDataset):
    def __getitem__(self, idx: int) -> SCRNASeqPerturbationDataSample:
        return dict(
            idx=idx, X=self.X[idx], D=self.D[idx], library_size=self.library_size[idx], qc=self.qc[idx], cfX=self.cfX[idx], deltaX=self.deltaX[idx], # deltaX = self.deltaX[idx],
        )
