...
from gpo_vae.models.utils.gumbel_softmax_bernoulli import (
...
class gpo_vae_CorrelatedNormalGuide(nn.Module):
    def __init__(
        self,
        n_latent: int,
        n_treatments: int,
        ...
        if knowledge_path is None:
            self.param_dict[f"q_mask_{logits_or_probs}"] = torch.nn.Parameter(
                mask_init * torch.ones((n_treatments, n_treatments)) # n_treatments, n_latent
            )
        ...
        self.embedding_encoder = get_likelihood_mlp(
            ...
            n_input=n_treatments + n_treatments, # n_latent + n_treatments
            n_output=n_treatments, # n_latent
        ...
        self.z_basal_encoder = get_likelihood_mlp(
            likelihood_key="normal",
            n_input=n_phenos if mean_field_encoder else n_phenos + n_treatments + n_latent, # n_phenos + n_latent + n_latent
            n_output=n_latent,
    ...
    def forward(
        ...
        if self.logits_or_probs == "logits":
            ...
            raise "input only logits or probs"
