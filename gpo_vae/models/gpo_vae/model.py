...
from gpo_vae.models.utils.mlp import LIKELIHOOD_KEY_DTYPE, get_likelihood_mlp


class gpo_vae_Model(torch.nn.Module):
    def __init__(
        self,
        n_latent: int,
        n_treatments: int,
        ...
        self.register_buffer("p_E_loc", torch.zeros((n_treatments, n_treatments))) # n_treatments, n_treatments
        self.register_buffer(
            "p_E_scale", embedding_prior_scale * torch.ones((n_treatments, n_treatments)) # n_treatments, n_treatments
        )
        self.register_buffer(
            f"p_mask_{logits_or_probs}", mask_prior * torch.ones((n_treatments, n_treatments)) # n_treatments, n_treatments
        )
        ...
        self.decoder = get_likelihood_mlp(
            ...
            n_input=n_latent if sum_mode else n_latent + n_treatments + n_latent, # n_latent + n_treatments + n_latent
            n_output=n_phenos,
    ...
    def forward(
        ...
        if self.logits_or_probs == "logits":
            generative_dists["p_mask"] = Bernoulli(logits=torch.logit(eval(f"self.p_mask_{self.logits_or_probs}")))
        elif self.logits_or_probs == "probs":
            generative_dists["p_mask"] = Bernoulli(probs=eval(f"self.p_mask_{self.logits_or_probs}"))
